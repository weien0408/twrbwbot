...
# 📜 賽季規則
## 🟢 全場可以使用
1. 階梯 | 2. 鑽石裝 | 3. 鑽石劍 | 4. 冰橋 | 5. 藍隊跟黃隊的島嶼
## 💎 鑽石 III 之後
1. 跳躍藥水 | 2. 速度藥水
## 🟢 綠寶 III 之後
1. 擊退棒
## ⚠️ 床被破壞後
1. 隱形藥水 | 2. 水桶 | 3. 終界珍珠 | 4. 火球炸鑽石島 | 5. 弓箭
## 🚫 全場禁止：黑曜石、活動物品
"""
...
async def update_member_visuals(member, elo, ign):
    ...
    try:
        ...
        print(f"無法修改暱稱 {member.name}: {e}")


    RANK_ROLES = {
        (0, 100): 1471938798194528326, 
        (100, 300): 1471938837218328730,
        (300, 500): 1471938873935270043, 
        (500, 700): 1471938889311719576,
        (700, 99999): 1471938907816988794
    }
    all_rank_ids = list(RANK_ROLES.values())
    target_id = next((rid for (low, high), rid in RANK_ROLES.items() if low <= elo < high), None)
    
   
    to_remove = [member.guild.get_role(rid) for rid in all_rank_ids if rid != target_id and member.guild.get_role(rid) in member.roles]
    if to_remove:
...
class SettlementView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction):
        ...
        for p in losers:
            if str(p.id) != mvp_id: # MVP 敗場不扣分
                p_db[str(p.id)]["elo"] = max(0, p_db[str(p.id)]["elo"] - 10)
...
@bot.tree.command(name="register", description="綁定 Hypixel 帳號並獲取身分組")
async def register(interaction: discord.Interaction, ign: str):
    ...
    if status == "SUCCESS":
        ...
        save_db(USER_DB, u_db); save_db(PLAYERS_DB, p_db)
        ...
        await interaction.followup.send(f"✅ 註冊成功！歡迎 `{ign}`，已為您開啟參賽權限。")
    else:
        msg = {
...
@bot.tree.command(name="setup_vc", description="設定排隊房門檻 (防虐菜)")
...
async def setup_vc(interaction: discord.Interaction, channel: discord.VoiceChannel, min_elo: int, max_elo: int):
    ...
    db = load_db("gated_vcs.json")
    db[str(channel.id)] = {"min": min_elo, "max": max_elo}
    save_db("gated_vcs.json", db)
...
@bot.tree.command(name="set_elo", description="手動調整玩家分數")
@app_commands.checks.has_permissions(administrator=True)
async def set_elo(interaction: discord.Interaction, member: discord.Member, value: int):
    db = load_db(PLAYERS_DB)
    if str(member.id) not in db: return await interaction.response.send_message("❌ 玩家未註冊。")
    db[str(member.id)]["elo"] = value
    save_db(PLAYERS_DB, db)
    await update_member_visuals(member, value, db[str(member.id)]["ign"])
    await interaction.response.send_message(f"✅ 已手動更新 {member.mention} 的分數為 `{value}`。")
